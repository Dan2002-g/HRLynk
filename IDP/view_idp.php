...
$idp_details = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $stmt_approval = $conn->prepare("
        SELECT approver.empname AS supervisor_name, idp.created_at AS approval_date, idp.status
        FROM idp
        JOIN users approver ON idp.approved_by = approver.userID
        WHERE idp.id = :id AND approver.roleID IN (1, 2, 5, 7, 9, 10)
    ");
    $stmt_approval->bindParam(':id', $id, PDO::PARAM_INT);
    $stmt_approval->execute();
    $approval_details = $stmt_approval->fetch(PDO::FETCH_ASSOC);
...
<p style="display: flex; justify-content: space-between; text-align: center; margin-top: 20px;">
    <span>
        <?php 
        if ($approval_details['status'] === 'Approved') {
            echo isset($approval_details['approval_date']) 
                ? date('F d, Y', strtotime($approval_details['approval_date'])) . "<br>" 
                : '';
            echo "Approved by: " . htmlspecialchars($approval_details['supervisor_name'] ?? '__________________');
        } else {
            echo "Approved by: __________________";
        }
        ?><br>
